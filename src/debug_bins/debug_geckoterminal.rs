...
async fn test_trending_pools(client: &Client, args: &Args) {
 ...
 println!("{} Expected data array", "".yellow());
              }
            }
            Err(e) => {
 println!("{} Parse error: {}", "".red(), e);
...
async fn test_ohlcv_endpoint(client: &Client, url: &str, args: &Args) {
 ...
 println!("{} No ohlcv_list in response", "".yellow());
                  if args.verbose {
                    println!(
 "Response: {}",
                      serde_json::to_string_pretty(&json).unwrap_or_default()
...
fn validate_ohlcv_structure(ohlcv_list: &[Value]) {
 ...
 println!("{} No candles in response", "".yellow());
    return;
  }
 ...
 println!("{} Some elements are not numbers", "".yellow());
        }
      } else {
        println!(
 ...
 println!("{} Timestamps are not monotonic", "".yellow());
  }
}

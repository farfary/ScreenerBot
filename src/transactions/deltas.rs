//! Subject-relative balance deltas — pure extraction from a decoded transaction.
//!
//! Mirrors the parsing prologue in `wallets::watch::classify` (account keys, pre/post
//! token balances filtered by owner) but keeps every mint's delta instead of
//! collapsing to one dominant swap leg. This is the ledger the positions module's
//! wallet-history reducer (`positions::ledger`) is built on: a wallet-derived
//! `External` position is only ever as good as these rows.
//!
...
use crate::transactions::analyzer::balance::account_keys_from_message;
use crate::transactions::analyzer::dex::extract_program_ids;
use crate::transactions::program_ids::detect_router_from_program_id;
...
pub fn extract_subject_deltas(
    ...
    let Ok(tx_data) = serde_json::from_value::<crate::rpc::TransactionDetails>(raw.clone()) else {
        return Vec::new();
    };
    ...
    let empty: Vec<crate::rpc::TokenBalance> = Vec::new();
...
mod tests {
    use super::*;
    use crate::transactions::program_ids::RAYDIUM_CPMM_PROGRAM_ID;
    use serde_json::json;

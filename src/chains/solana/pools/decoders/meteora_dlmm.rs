...
use crate::chains::solana::constants::METEORA_DLMM_PROGRAM_ID;
    ...
    fn decode_and_calculate(
        ...
        if sol_decimals > 18 || token_decimals > 18 {
            logger::error(
                LogTag::PoolDecoder,
                &format!(
                    "Meteora DLMM: Decimals too large: sol={}, token={}",
...
impl MeteoraDlmmDecoder {
    ...
    fn calculate_dlmm_price(dlmm_info: &DlmmPoolInfo, token_mint: &str) -> Option<f64> {
        ...
        let token_x_decimals = if dlmm_info.token_x_mint == SOL_MINT {
            ...
            get_cached_decimals(crate::chains::ChainId::Solana, &dlmm_info.token_x_mint)
                .unwrap_or(6)
        };
        let token_y_decimals = if dlmm_info.token_y_mint == SOL_MINT {
            SOL_DECIMALS
        } else {
            get_cached_decimals(crate::chains::ChainId::Solana, &dlmm_info.token_y_mint)
                .unwrap_or(6)
        };
...
mod tests {
    ...
    fn short_data_is_rejected_without_panicking() {
        assert!(MeteoraDlmmDecoder::parse_dlmm_pool(&[]).is_none());
        assert!(MeteoraDlmmDecoder::parse_dlmm_pool(&vec![0u8; 215]).is_none());
        assert!(MeteoraDlmmDecoder::extract_reserve_accounts(&vec![0u8; 215]).is_none());
    }

    #[test]
    fn valid_layout_decodes_mints_reserves_and_bin_state() {
        ...
        data[76..80].copy_from_slice(&(-485i32).to_le_bytes());
        data[80..82].copy_from_slice(&30u16.to_le_bytes());
        data[88..120].copy_from_slice(token_x_mint.as_ref());
        data[120..152].copy_from_slice(token_y_mint.as_ref());
        data[152..184].copy_from_slice(reserve_x.as_ref());
        data[184..216].copy_from_slice(reserve_y.as_ref());

        let info = MeteoraDlmmDecoder::parse_dlmm_pool(&data)
            .expect("well-formed DLMM account data must decode");

        assert_eq!(info.token_x_mint, token_x_mint.to_string());
        assert_eq!(info.token_y_mint, token_y_mint.to_string());
        assert_eq!(info.reserve_x, reserve_x.to_string());
        assert_eq!(info.reserve_y, reserve_y.to_string());
        assert_eq!(info.active_id, -485);
        assert_eq!(info.bin_step, 30);

        let reserves = MeteoraDlmmDecoder::extract_reserve_accounts(&data)
            .expect("reserve extraction must succeed for the same layout");
        assert_eq!(reserves, vec![reserve_x.to_string(), reserve_y.to_string()]);
    }
}

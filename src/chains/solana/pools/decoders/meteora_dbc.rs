...
impl PoolDecoder for MeteoraDbcDecoder {
    ...
    fn decode_and_calculate(
        ...
        // Decimals
        let token_decimals = match get_cached_decimals(crate::chains::ChainId::Solana, &token_mint)
        {
            Some(d) => d,
            None => {
                return None;
...
mod tests {
    ...
    #[test]
    fn read_u128_le_and_u64_le_reject_short_slices() {
        assert!(read_u128_le(&[0u8; 15]).is_err());
        assert!(read_u64_le(&[0u8; 7]).is_err());
        assert_eq!(read_u128_le(&42u128.to_le_bytes()).unwrap(), 42);
        assert_eq!(read_u64_le(&7u64.to_le_bytes()).unwrap(), 7);
    }

    #[test]
    fn sqrt_price_short_data_falls_back_to_scan_and_then_none() {
        assert_eq!(extract_sqrt_price_from_pool_data(&[]), None);
        // Below the primary offset's read window (296 bytes), so the fallback
        // scan runs; all-zero data has no plausible nonzero u128 to find there.
        assert_eq!(extract_sqrt_price_from_pool_data(&vec![0u8; 200]), None);
    }

    #[test]
    fn sqrt_price_primary_offset_returns_even_a_zero_value() {
        // The primary-offset read (data.len() >= 296) does not itself validate
        // non-zero-ness — only the fallback scan does.
        assert_eq!(extract_sqrt_price_from_pool_data(&vec![0u8; 296]), Some(0));
    }

    #[test]
    fn sqrt_price_is_read_from_the_primary_offset() {
        let mut data = vec![0u8; 296];
        data[280..296].copy_from_slice(&123_456_789u128.to_le_bytes());
        assert_eq!(extract_sqrt_price_from_pool_data(&data), Some(123_456_789));
    }

    #[test]
    fn extract_reserve_accounts_rejects_short_data() {
        assert!(MeteoraDbcDecoder::extract_reserve_accounts(&[]).is_none());
        assert!(MeteoraDbcDecoder::extract_reserve_accounts(&vec![0u8; 423]).is_none());
    }

    /// Base58 length depends on the numeric value, not just "no leading zero
    /// byte" — search a small deterministic range for a 44-char address so
    /// the fixture passes the decoder's length check like a real pubkey does.
    fn find_44_char_pubkey(seed_start: u8) -> Pubkey {
        (seed_start..)
            .map(|seed| {
                Pubkey::new_from_array(std::array::from_fn(|i| (i as u8).wrapping_add(seed)))
            })
            .find(|pk| pk.to_string().len() == 44)
            .expect("a 44-char pubkey exists within a small search range")
    }

    #[test]
    fn extract_reserve_accounts_finds_the_first_plausible_pubkey_pair() {
        let vault1 = find_44_char_pubkey(1);
        let vault2 = find_44_char_pubkey(64);
        let mut data = vec![0u8; 424];
        data[32..64].copy_from_slice(vault1.as_ref());
        data[64..96].copy_from_slice(vault2.as_ref());

        let reserves = MeteoraDbcDecoder::extract_reserve_accounts(&data)
            .expect("a well-formed vault pair at offset 32 must be found");
        assert_eq!(reserves, vec![vault1.to_string(), vault2.to_string()]);
    }
}

...
mod tests {
    ...
    fn short_data_is_rejected_without_panicking() {
        assert!(MeteoraDammDecoder::parse_damm_pool(&[]).is_none());
        assert!(MeteoraDammDecoder::parse_damm_pool(&vec![0u8; 1111]).is_none());
        assert!(MeteoraDammDecoder::extract_reserve_accounts(&vec![0u8; 1111]).is_none());
    }

    #[test]
    fn valid_layout_decodes_mints_vaults_fees_liquidity_and_price() {
        ...
        data[168..200].copy_from_slice(token_a_mint.as_ref());
        data[200..232].copy_from_slice(token_b_mint.as_ref());
        data[232..264].copy_from_slice(token_a_vault.as_ref());
        data[264..296].copy_from_slice(token_b_vault.as_ref());
        data[392..400].copy_from_slice(&11u64.to_le_bytes());
        data[400..408].copy_from_slice(&22u64.to_le_bytes());
        data[408..416].copy_from_slice(&33u64.to_le_bytes());
        data[416..424].copy_from_slice(&44u64.to_le_bytes());
        data[296..312].copy_from_slice(&500u128.to_le_bytes());
        data[456..472].copy_from_slice(&999u128.to_le_bytes());

        let info = MeteoraDammDecoder::parse_damm_pool(&data)
            .expect("well-formed DAMM account data must decode");

        assert_eq!(info.token_a_mint, token_a_mint.to_string());
        assert_eq!(info.token_b_mint, token_b_mint.to_string());
        assert_eq!(info.token_a_vault, token_a_vault.to_string());
        assert_eq!(info.token_b_vault, token_b_vault.to_string());
        assert_eq!(info.protocol_a_fee, 11);
        assert_eq!(info.protocol_b_fee, 22);
        assert_eq!(info.partner_a_fee, 33);
        assert_eq!(info.partner_b_fee, 44);
        assert_eq!(
            info.liquidity, 500,
            "prefers the @296 liquidity slot when non-zero"
        );
        assert_eq!(
            info.sqrt_price, 999,
            "prefers the @456 sqrt_price slot when non-zero"
        );

        let reserves = MeteoraDammDecoder::extract_reserve_accounts(&data)
            .expect("reserve extraction must succeed for the same layout");
        assert_eq!(
            reserves,
            vec![token_a_vault.to_string(), token_b_vault.to_string()]
        );
    }
}

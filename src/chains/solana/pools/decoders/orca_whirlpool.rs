...
use crate::chains::solana::constants::ORCA_WHIRLPOOL_PROGRAM_ID;
...
mod tests {
    ...
    fn short_data_is_rejected_without_panicking() {
        assert!(OrcaWhirlpoolDecoder::parse_whirlpool_data(&[]).is_none());
        assert!(OrcaWhirlpoolDecoder::parse_whirlpool_data(&vec![0u8; 652]).is_none());
        assert!(OrcaWhirlpoolDecoder::extract_reserve_accounts(&vec![0u8; 652]).is_none());
    }

    #[test]
    fn valid_layout_decodes_mints_vaults_liquidity_and_sqrt_price() {
        ...
        data[49..65].copy_from_slice(&123_456_789u128.to_le_bytes());
        data[65..81].copy_from_slice(&987_654_321u128.to_le_bytes());
        data[101..133].copy_from_slice(token_mint_a.as_ref());
        data[133..165].copy_from_slice(token_vault_a.as_ref());
        data[181..213].copy_from_slice(token_mint_b.as_ref());
        data[213..245].copy_from_slice(token_vault_b.as_ref());

        let info = OrcaWhirlpoolDecoder::parse_whirlpool_data(&data)
            .expect("well-formed Whirlpool account data must decode");

        assert_eq!(info.token_mint_a, token_mint_a.to_string());
        assert_eq!(info.token_vault_a, token_vault_a.to_string());
        assert_eq!(info.token_mint_b, token_mint_b.to_string());
        assert_eq!(info.token_vault_b, token_vault_b.to_string());
        assert_eq!(info.liquidity, 123_456_789);
        assert_eq!(info.sqrt_price, 987_654_321);

        let reserves = OrcaWhirlpoolDecoder::extract_reserve_accounts(&data)
            .expect("reserve extraction must succeed for the same layout");
        assert_eq!(
            reserves,
            vec![token_vault_a.to_string(), token_vault_b.to_string()]
        );
    }
}

...
mod tests {
    ...
    #[test]
    fn wrong_size_data_is_rejected_without_panicking() {
        assert!(FluxbeamAmmDecoder::parse_fluxbeam_pool(&[]).is_none());
        assert!(FluxbeamAmmDecoder::parse_fluxbeam_pool(&vec![0u8; 323]).is_none());
        assert!(FluxbeamAmmDecoder::parse_fluxbeam_pool(&vec![0u8; 325]).is_none());
        assert!(FluxbeamAmmDecoder::extract_reserve_accounts(&vec![0u8; 323]).is_none());
    }

    #[test]
    fn valid_layout_decodes_mints_and_vaults_at_fixed_offsets() {
        ...
        data[35..67].copy_from_slice(token_a_vault.as_ref());
        data[67..99].copy_from_slice(token_b_vault.as_ref());
        data[131..163].copy_from_slice(token_a_mint.as_ref());
        data[163..195].copy_from_slice(token_b_mint.as_ref());

        let info = FluxbeamAmmDecoder::parse_fluxbeam_pool(&data)
            .expect("well-formed FluxBeam pool data must decode");
        ...
        let reserves = FluxbeamAmmDecoder::extract_reserve_accounts(&data)
            .expect("reserve account extraction must succeed for the same layout");
        assert_eq!(
            reserves,

...
use crate::chains::solana::constants::RAYDIUM_CLMM_PROGRAM_ID;
        ...
        for i in 0..3 {
            ...
            if remaining_space < 100 {
                ...
                reward_infos.push(RewardInfo {
                    ...
                    reward_claimed: 0,
                    token_mint: "11111111111111111111111111111111".to_owned(),
                    token_vault: "11111111111111111111111111111111".to_owned(),
                    authority: "11111111111111111111111111111111".to_owned(),
...
mod tests {
    ...
    fn short_data_is_rejected_without_panicking() {
        assert!(RaydiumClmmDecoder::parse_clmm_pool(&[]).is_none());
        assert!(RaydiumClmmDecoder::parse_clmm_pool(&vec![0u8; 1199]).is_none());
        assert!(RaydiumClmmDecoder::extract_reserve_accounts(&vec![0u8; 799]).is_none());
    }

    #[test]
    fn valid_layout_decodes_mints_vaults_and_fee_fields() {
        ...
        data[8] = 254; // bump
                       // amm_config [9,41), owner [41,73) left zeroed (unused by the test).
        data[73..105].copy_from_slice(token_mint_0.as_ref());
        data[105..137].copy_from_slice(token_mint_1.as_ref());
        data[137..169].copy_from_slice(token_vault_0.as_ref());
        data[169..201].copy_from_slice(token_vault_1.as_ref());
        data[1032..1040].copy_from_slice(&777u64.to_le_bytes()); // total_fees_token_0

        let info = RaydiumClmmDecoder::parse_clmm_pool(&data)
            .expect("well-formed CLMM account data must decode");

        assert_eq!(info.token_mint_0, token_mint_0.to_string());
        assert_eq!(info.token_mint_1, token_mint_1.to_string());
        assert_eq!(info.token_vault_0, token_vault_0.to_string());
        assert_eq!(info.token_vault_1, token_vault_1.to_string());
        assert_eq!(info.total_fees_token_0, 777);

        let reserves = RaydiumClmmDecoder::extract_reserve_accounts(&data)
            .expect("reserve extraction must succeed for the same layout");
        assert_eq!(
            reserves,
            vec![token_vault_0.to_string(), token_vault_1.to_string()]
        );
    }
}

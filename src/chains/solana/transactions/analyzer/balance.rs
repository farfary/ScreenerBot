...
async fn extract_token_balance_changes(
    ...
    let mut pre_map: HashMap<(u32, String), &crate::chains::solana::rpc::UiTokenAmount> =
        HashMap::new();
    let mut post_map: HashMap<(u32, String), &crate::chains::solana::rpc::UiTokenAmount> =
        HashMap::new();
    ...
    for (account_index, mint) in all_keys {
        ...
        if (pre_ui - post_ui).abs() > f64::EPSILON {
            ...
            let decimals = post_amount
                ...
                .unwrap_or_else(|| {
                    get_cached_decimals(crate::chains::ChainId::Solana, &mint).unwrap_or(9)
                });
...
async fn detect_mev_tips_from_instructions(
    tx_data: &crate::chains::solana::rpc::TransactionDetails,
) -> f64 {

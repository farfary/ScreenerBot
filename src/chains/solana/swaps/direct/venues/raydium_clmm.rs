...
//!
//! The quote solves ONE constant-liquidity step from the current price. That is
//! exact while the swap stays inside the current tick range, which a
//! bot-sized trade against a real pool essentially always does. Beyond a tick
//! boundary the active liquidity changes and this estimate can be too high.
//!
//! Two things keep that from costing money. The estimate is refused outright
//! when the step would leave the current tick array, and every swap is
//! simulated before it is sent — an unsatisfiable `min_out` is rejected by a
//! node for free rather than by the pool for a priority fee. What this venue
//! must get exactly right is the ACCOUNTS and the INSTRUCTION, and those come
//! from the chain's own bitmap rather than from arithmetic.

use super::clmm_ticks::{
    bitmap_extension_address, tick_array_address, tick_array_start_index, TickArrayBitmap,
    TICK_ARRAYS_PER_SWAP,
};
use super::layout::{i32_at, pubkey_at, token_account_amount, u128_at, u16_at, u32_at, u8_at};
use super::math::{fee_amount, mul_div_ceil, mul_div_floor};
...
impl PoolVenue for RaydiumClmmVenue {
    ...
    async fn load(
        ...
        let mut bitmap = TickArrayBitmap::from_pool_state(&pool_account.data).ok_or_else(|| {
            DirectSwapError::PoolUndecodable {
                pool: *pool,
...
impl ClmmMarket {
    ...
    /// The sqrt price this step ends at, and the output it produces, for one
    /// constant-liquidity step from the current price.
    ///
    /// Selling token_0 moves the price DOWN and pays out token_1; selling
    /// token_1 moves it UP and pays out token_0. Both directions round the
    /// output DOWN and the ending price AGAINST the trader, matching the
    /// programme and never over-promising by a raw unit.
    fn step(&self, zero_for_one: bool, amount_in: u64) -> DirectSwapResult<(u128, u64)> {
        ...
        if liquidity == 0 || sqrt_price == 0 {
            return Err(DirectSwapError::InsufficientLiquidity {
                ...
                amount_in,
                detail: "no liquidity in range at the current tick".to_owned(),
            });
        }
        ...
        if zero_for_one {
            // sqrt_next = L * sqrt / (L + amount * sqrt / 2^64)
            let numerator =
                liquidity
                    .checked_shl(64)
            ...
            let out = mul_div_floor(liquidity, sqrt_price.saturating_sub(sqrt_next), 1u128 << 64)
                .ok_or_else(overflow)?;
            Ok((sqrt_next, out.min(u64::MAX as u128) as u64))
        } else {
            // sqrt_next = sqrt + amount * 2^64 / L
            let delta = mul_div_floor(amount, 1u128 << 64, liquidity).ok_or_else(overflow)?;
            ...
            let spread = sqrt_next.saturating_sub(sqrt_price);
            let intermediate = mul_div_floor(numerator, spread, sqrt_next).ok_or_else(overflow)?;
            let out = intermediate / sqrt_price;
            Ok((sqrt_next, out.min(u64::MAX as u128) as u64))
        }
    }

    /// The sqrt price at the edge of the current tick array, in the direction
    /// the swap is moving. A step past this leaves the range the quote can speak
    /// for.
    ///
    /// `f64` is deliberate and safe here: this is a GUARD, not an amount. Its
    /// only job is to refuse a size whose step would leave the current array,
    /// and a relative error of 1e-16 cannot flip that verdict for any size worth
    /// trading. Nothing derived from this value reaches the instruction.
    fn range_edge_sqrt_price(&self, zero_for_one: bool) -> u128 {
        let start = tick_array_start_index(self.state.tick_current, self.state.tick_spacing);
        let edge_tick = if zero_for_one {
            start
        } else {
            start + super::clmm_ticks::ticks_per_array(self.state.tick_spacing)
        };
        let sqrt = 1.0001_f64.powf(edge_tick as f64 / 2.0) * (2.0_f64).powi(64);
        if !sqrt.is_finite() || sqrt <= 0.0 {
            return if zero_for_one { 0 } else { u128::MAX };
        }
        sqrt as u128
    }
...
impl PoolMarket for ClmmMarket {
    ...
    fn quote(&self, input_mint: &Pubkey, amount_in: u64) -> DirectSwapResult<VenueQuote> {
        ...
        let (sqrt_next, gross_out) = self.step(input_is_0, swappable)?;

        // Refuse a size the single-step quote cannot speak for. Past the edge of
        // the current tick array the active liquidity changes, and an estimate
        // that assumed it constant would be too high — which becomes a `min_out`
        // the pool cannot meet.
        let edge = self.range_edge_sqrt_price(input_is_0);
        let leaves_range = if input_is_0 {
            sqrt_next < edge
        } else {
            sqrt_next > edge
        };
        if leaves_range {
            return Err(DirectSwapError::InsufficientLiquidity {
                pool: self.state.pool,
                amount_in,
                detail: "the size would move the price out of the current tick range, which this \
                         venue cannot quote exactly"
                    .to_owned(),
            });
        }

...
mod tests {
    ...
    #[test]
    fn success_response_decodes_quote_and_raw_transaction() {
        let json = r#"{
            "code": 0,
            "msg": "success",
            "tid": "abc123",
            "data": {
                ...
                "raw_tx": {
                    ...
                    "recentBlockhash": "Blockhash1111111111111111111111111111111",
                    "version": "v0"
                },
                "amount_in_usd": "100.0",
                "amount_out_usd": "99.5",
        ...
        let response: GMGNApiResponse =
            serde_json::from_str(json).expect("well-formed GMGN success response must decode");
        assert_eq!(response.code, 0);
        let data = response.data.expect("success response carries data");
        assert_eq!(data.quote.out_amount, "123456789");
        assert_eq!(data.quote.in_decimals, 9);
        assert_eq!(data.raw_tx.swap_transaction, "base64tx==");
        assert_eq!(data.sol_cost.as_deref(), Some("0.001"));
    }

    #[test]
    fn error_response_decodes_with_no_data() {
        let json = r#"{"code": 429, "msg": "rate limited", "tid": null, "data": null}"#;

        let response: GMGNApiResponse =
            serde_json::from_str(json).expect("well-formed GMGN error response must decode");
        assert_eq!(response.code, 429);
        assert_eq!(response.msg, "rate limited");
        assert!(response.data.is_none());
    }

    #[test]
    fn malformed_response_is_rejected() {
        let result: std::result::Result<GMGNApiResponse, _> =
            serde_json::from_str("{\"not\": \"a gmgn response\"}");
